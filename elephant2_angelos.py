...
PRINT_THREAD_INTERVAL = 2  # seconds

flows = {}

class FuncThread(threading.Thread):
    def __init__(self, target, lock, *args):
        self._target = target
        self._args = args
        self.lock = lock
    ...
    def run(self):
        while 1:
            ...
            self.lock.release()
            time.sleep(PRINT_THREAD_INTERVAL)

def startServer():
    subprocess.call(["python","../cosign-emulation/APIs/virtual_observer_server.py"])

def listener_put(value,type):
    global FLOW_TIMEOUT, ELEPHENT_BANDWIDTH, ELEPHENT_DURATION
    if type == "FLOW_TIMEOUT":
        FLOW_TIMEOUT = value
    elif type == "ELEPHANT_DURATION":
        ELEPHENT_DURATION = value
    elif type == "ELEPHANT_BANDWIDTH":
        ELEPHENT_BANDWIDTH = value

def listener_get(type):
    if type == "FLOW_TIMEOUT":
        return FLOW_TIMEOUT
    elif type == "ELEPHANT_DURATION":
        return ELEPHENT_DURATION
    elif type == "ELEPHANT_BANDWIDTH":
        return ELEPHENT_BANDWIDTH
...
if __name__ == "__main__":
    lock = threading.Lock()
    t1 = FuncThread(startServer,lock)
    t1.setDaemon(True)
    t1.start()
